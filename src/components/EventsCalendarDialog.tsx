...
import { addDays, endOfDay, endOfMonth, endOfWeek, format, isSameDay, isSameMonth, isToday, startOfDay, startOfMonth, startOfWeek, differenceInMinutes, max as dfMax, min as dfMin } from "date-fns";
...
function CalendarBody({ onClose }: { onClose?: () => void }) {
  ...
  const rangeStart = useMemo(() => {
    ...
    if (view === "week") return startOfWeek(anchor, { weekStartsOn: 0 });
    return startOfMonth(anchor);
  }, [anchor, view]);
  const rangeEnd = useMemo(() => {
    if (view === "day") return endOfDay(anchor);
    if (view === "week") return endOfWeek(anchor, { weekStartsOn: 0 });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6">
      <div className="space-y-3">
        ...
        <div className="rounded-lg border p-4">
          ...
          <CreateEventForm defaultDate={selected} onCreate={onCreated} presetStart={draftStart ?? undefined} presetEnd={draftEnd ?? undefined} onClearPreset={() => { setDraftStart(null); setDraftEnd(null); }} />
        </div>
      </div>
...
function CreateEventForm({ defaultDate, onCreate, presetStart, presetEnd, onClearPreset }: { defaultDate: Date; onCreate: (data: { title: string; description?: string; start: Date; end: Date; allDay: boolean; reminder: number | null }) => void; presetStart?: Date; presetEnd?: Date; onClearPreset?: () => void }) {
  ...
  const [reminder, setReminder] = useState<number | "none">("none");
  ...
  setTitle(""); setDesc(""); setAllDay(false); setStartTime("09:00"); setEndTime("10:00"); setReminder("none");
  onClearPreset?.();
  };
...
function layoutDay(events: DbEvent[], day: Date) {
  ...
  const dayEvents = events
    ...
    .map((e) => {
      ...
      const top = (s - start) / 60000 * MINUTE_PX;
      const height = Math.max(20, (ee - s) / 60000 * MINUTE_PX);
...
function TimeGrid({ days, events, onSlotClick, onUpdate, onDelete }: { days: Date[]; events: DbEvent[]; onSlotClick: (start: Date) => void; onUpdate: (id: string, patch: any) => void; onDelete: (id: string) => void }) {
  ...
  const laidByDay = useMemo(() => {
    return days.map((d) => layoutDay(events, d));
  }, [days.map((d) => d.toDateString()).join("|"), events.map((e) => e.id + e.start_iso + e.end_iso).join("|")]);

  const handleSlot = (day: Date, hour: number, yWithin?: number) => {
    const start = new Date(day);
    start.setHours(hour, yWithin ? Math.round(yWithin / MINUTE_PX) : 0, 0, 0);
    onSlotClick(start);
  };
